...
def llm(query, history=[], user_stop_words=[]):  # 调用api_server
    ...
    try:
        ...
        # print(resp)
        content = (
            resp.get("Data", {}).get("Choices", [])[0].get("Message", {}).get("Content")
        )
        return content
...
os.environ["TAVILY_API_KEY"] = (
    "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"  # travily搜索引擎api key
)
...
while True:
    query = input("query:")
    success, result, my_history = agent_execute_with_retry(
        query, chat_history=my_history
    )
