...
class CustomArguments:
    section: str = field(metadata={"help": "训练任务：[pretrain, sft, dpo]"})
    train_mode: str = field(metadata={"help": "训练方式：[full, lora, qlora]"})
    max_seq_length: int = field(metadata={"help": "输入最大长度"})
    data: str = field(metadata={"help": "训练集。如果task_type=pretrain，请指定文件夹，将扫描其下面的所有jsonl文件"})
